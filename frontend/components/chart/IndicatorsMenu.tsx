...
export default function IndicatorsMenu({ active, onToggle, onAdd, pinned, onTogglePin, showLabel = false }: IndicatorsMenuProps) {
  const [open, setOpen] = useState(false);
  const [hover, setHover] = useState(false);
  const buttonRef = useRef<HTMLButtonElement>(null);

  const hasUnrepresentedActive = showLabel ? false : [...active].some((id) => !pinned.has(id));
        ...
        className={
          showLabel
            ? `flex h-8 shrink-0 items-center gap-1.5 rounded-full border px-3 transition-colors ${
                hasUnrepresentedActive ? "border-violet-500 bg-violet-500 text-on-accent" : "border-border bg-field text-muted hover:bg-violet-500/10 hover:text-fg"
              }`
            : `flex h-7 items-center gap-1 rounded px-1.5 transition-colors ${
                hasUnrepresentedActive ? "bg-violet-500 text-on-accent" : "text-muted hover:bg-violet-500/20 hover:text-fg"
              }`
        }
      >
        <TrendingUp size={16} strokeWidth={2} />
        {showLabel && <span className="whitespace-nowrap text-xs">Indicators</span>}
        {active.size > 0 && <span className="text-[10px] font-semibold tabular-nums">{active.size}</span>}
        <ChevronDown size={12} strokeWidth={2} className="opacity-70" />
      </button>
      {!showLabel && <ToolbarTooltip label="Indicators" hover={hover && !open} anchorRef={buttonRef} />}
      <DropdownPanel open={open} anchorRef={buttonRef} align="right">
        <div className="w-48 rounded-md border border-border bg-panel py-1 shadow-lg">
