...
from app.schemas import Account, OrderResponse, PortfolioHistory, PortfolioPoint, Position
...
def test_create_order_submits_and_logs_intent(client):
    with (
        patch("app.routers.trading.alpaca_client.submit_order", return_value=_order_response()) as mock_submit,
        patch("app.routers.trading.log_order_intent", new=AsyncMock()) as mock_log,
    ):
        resp = client.post(
            "/api/trading/orders", json={"symbol": "AAPL", "qty": 10, "side": "buy", "type": "market"}
        )
...
def test_create_order_maps_runtime_error_to_503(client):
    with patch("app.routers.trading.alpaca_client.submit_order", side_effect=RuntimeError("no creds")):
        resp = client.post(
            "/api/trading/orders", json={"symbol": "AAPL", "qty": 10, "side": "buy", "type": "market"}
        )
...
def test_create_order_maps_value_error_to_400(client):
    with patch("app.routers.trading.alpaca_client.submit_order", side_effect=ValueError("bad order")):
        resp = client.post(
            "/api/trading/orders", json={"symbol": "AAPL", "qty": 10, "side": "buy", "type": "market"}
        )
...
def test_create_order_rejects_non_positive_qty(client):
    resp = client.post("/api/trading/orders", json={"symbol": "AAPL", "qty": 0, "side": "buy", "type": "market"})
    assert resp.status_code == 422


def test_positions_returns_alpaca_positions(client):
    position = Position(symbol="AAPL", qty=10, avg_entry_price=100.0, market_value=1050.0, unrealized_pl=50.0)
...
def test_account_maps_runtime_error_to_503(client):
    with patch("app.routers.trading.alpaca_client.get_account", side_effect=RuntimeError("no creds")):
        resp = client.get("/api/trading/account")
    assert resp.status_code == 503


def test_portfolio_history_returns_points(client):
    history = PortfolioHistory(base_value=100_000.0, points=[PortfolioPoint(time=1, equity=101_000.0, profit_loss=1000.0)])
