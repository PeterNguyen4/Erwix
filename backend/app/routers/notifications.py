...
from app.models import AlpacaAccount, MarketInsightCache, NotificationDismissal, StrategyNote, UserPreference
...
async def _dismissal_map(db: AsyncSession, user_id: int) -> dict[str, NotificationDismissal]:
    rows = (
        await db.scalars(select(NotificationDismissal).where(NotificationDismissal.user_id == user_id))
    ).all()
    return {r.notification_key: r for r in rows}


def _state(dismissals: dict[str, NotificationDismissal], key: str, type_: str) -> NotificationDismissal | None:
    ...
    if row.dismissed_at is not None and type_ in PERSISTENT_TYPES:
        if datetime.now(timezone.utc) - row.dismissed_at > DISMISS_EXPIRY:
            return None
    return row
...
async def _build_items(db: AsyncSession, user_id: int) -> list[NotificationOut]:
    ...
    has_strategy = await db.scalar(select(StrategyNote).where(StrategyNote.user_id == user_id).limit(1))
...
async def _get_or_create(db: AsyncSession, user_id: int, key: str) -> NotificationDismissal:
    row = await db.scalar(
        select(NotificationDismissal).where(
            NotificationDismissal.user_id == user_id, NotificationDismissal.notification_key == key
        )
    )
    if row is None:
