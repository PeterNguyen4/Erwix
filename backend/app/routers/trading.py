...
@router.get("/positions", response_model=list[Position], dependencies=[Depends(_read_rate_limit)])
@alpaca_errors(logger)
...
@router.get("/portfolio/history", response_model=PortfolioHistory, dependencies=[Depends(_read_rate_limit)])
@alpaca_errors(logger)
